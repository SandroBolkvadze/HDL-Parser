...
- **Parsing and Building** (`engine` directory): `ChipParser` parses `.hdl` file into `ChipDescription`. `ChipBuilder` utilizes `ChipParser` and `Loader` (for reading files) to build recursive representation of chip given its name.  


- **Chip simulation** (`chips` directory): Every chip - Atomic or Circuit - conforms to same Chip Protocol which defines following method:
